...
from resnet import resnet_18 ,embedding_discriminator
...
class DG_frame(nn.Module):
    def __init__(self,out_channel):
        ...
        self.outchannels = out_channel
        self.resnet = resnet_18(outchannels=out_channel)
        self.ce = nn.CrossEntropyLoss()
    def forward(self, x, labels = None,constant = None):

        output = self.resnet(x,constant) #[batch,num_domain]
        loss = self.ce(output, labels)
        prec = accuracy(output.detach(), labels.detach(), topk=(1,))[0]
        rec = 0
        return loss, rec ,prec
...
'''class Frame_Encoder_1(nn.Module):
    def __init__(self,C):
        ...
        self.encoder1 = nn.Sequential(
            # Encoder
            # input (b, 1536, 202)
            nn.Conv1d(in_channels=C, out_channels=1024, kernel_size=3, stride=1, padding=1, bias=False,dilation=1),
            nn.ReLU(),
            nn.BatchNorm1d(1024),
            nn.Conv1d(1024, 512, kernel_size=3, stride=2, padding=1, bias=False,dilation=1),
            nn.ReLU(),
            nn.BatchNorm1d(512),
            nn.Conv1d(512, 256, kernel_size=3, stride=1, padding=1, bias=False,dilation=1),
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Conv1d(256, 192, kernel_size=3, stride=2, padding=1, bias=False,dilation=1),
            nn.ReLU(),
            nn.BatchNorm1d(192),
            # output (b, 128, 51)
        )
...
class Frame_Decoder_1(nn.Module):
    def __init__(self,C):
        ...
        self.decoder1 = nn.Sequential(
            nn.ConvTranspose1d(192, 256, kernel_size=3, stride=2, padding=1,  bias=False, dilation=1),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.ConvTranspose1d(256, 512, kernel_size=3, stride=1, padding=1, bias=False, dilation=1),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.ConvTranspose1d(512, 1024, kernel_size=3, stride=2, padding=1,output_padding=1, bias=False, dilation=1),
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.ConvTranspose1d(1024, out_channels=C, kernel_size=3, stride=1, padding=1, bias=False, dilation=1),
            nn.ReLU()
            # output (b, 512, 302)
        )
...
class Frame_Encoder_2(nn.Module):
    def __init__(self,C):
        ...
        self.encoder2 = nn.Sequential(
            # Encoder
            # input (b, 1536, 202)
            nn.Conv1d(in_channels=C, out_channels=1024, kernel_size=3, stride=1, padding=2, bias=False,dilation=2),
            nn.ReLU(),
            nn.BatchNorm1d(1024),
            nn.Conv1d(1024, 512, kernel_size=3, stride=2, padding=2, bias=False,dilation=2),
            nn.ReLU(),
            nn.BatchNorm1d(512),
            nn.Conv1d(512, 256, kernel_size=3, stride=1, padding=2, bias=False,dilation=2),
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Conv1d(256, 192, kernel_size=3, stride=2, padding=2, bias=False,dilation=2),
            nn.ReLU(),
            nn.BatchNorm1d(192),
            # output (b, 128, 51)
        )
...
class Frame_Decoder_2(nn.Module):
    def __init__(self,C):
        ...
        self.decoder2 = nn.Sequential(
            nn.ConvTranspose1d(192, 256, kernel_size=3, stride=2, padding=2,  bias=False, dilation=2),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.ConvTranspose1d(256, 512, kernel_size=3, stride=1, padding=2, bias=False, dilation=2),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.ConvTranspose1d(512, 1024, kernel_size=3, stride=2, padding=2,output_padding=1, bias=False, dilation=2),
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.ConvTranspose1d(1024, out_channels=C, kernel_size=3, stride=1, padding=2, bias=False, dilation=2),
            nn.ReLU()
            # output (b, 512, 302)
        )
...
class Frame_Encoder_3(nn.Module):
    def __init__(self,C):
        ...
        self.encoder3 = nn.Sequential(
            # Encoder
            # input (b, 1536, 202)
            nn.Conv1d(in_channels=C, out_channels=1024, kernel_size=3, stride=1, padding=3, bias=False,dilation=3),
            nn.ReLU(),
            nn.BatchNorm1d(1024),
            nn.Conv1d(1024, 512, kernel_size=3, stride=2, padding=3, bias=False,dilation=3),
            nn.ReLU(),
            nn.BatchNorm1d(512),
            nn.Conv1d(512, 256, kernel_size=3, stride=1, padding=3, bias=False,dilation=3),
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Conv1d(256, 192, kernel_size=3, stride=2, padding=3, bias=False,dilation=3),
            nn.ReLU(),
            nn.BatchNorm1d(192),
            # output (b, 128, 51)
        )
...
class Frame_Decoder_3(nn.Module):
    def __init__(self,C):
        ...
        self.decoder3 = nn.Sequential(
            nn.ConvTranspose1d(192, 256, kernel_size=3, stride=2, padding=3,  bias=False, dilation=3),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.ConvTranspose1d(256, 512, kernel_size=3, stride=1, padding=3, bias=False, dilation=3),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.ConvTranspose1d(512, 1024, kernel_size=3, stride=2, padding=3,output_padding=1, bias=False, dilation=3),
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.ConvTranspose1d(1024, out_channels=C, kernel_size=3, stride=1, padding=3, bias=False, dilation=3),
            nn.ReLU()
            # output (b, 512, 302)
        )
...
class frame_discriminator(nn.Module):
    def __init__(self):
        ...
        self.discriminator = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(512, 2, kernel_size=1, stride=1, padding=1, bias=False),
        )
...
class DG_frame(nn.Module):
    def __init__(self,C):
        super(DG_frame, self).__init__()
        self.C = C
        self.frame_encoder1 = Frame_Encoder_1(self.C)
        self.frame_decoder1 = Frame_Decoder_1(self.C)
        self.frame_discriminator1 = frame_discriminator()

        self.frame_encoder2 = Frame_Encoder_2(self.C)
        self.frame_decoder2 = Frame_Decoder_2(self.C)
        self.frame_discriminator2 = frame_discriminator()

        self.frame_encoder3 = Frame_Encoder_3(self.C)
        self.frame_decoder3 = Frame_Decoder_3(self.C)
        self.frame_discriminator3 = frame_discriminator()
